...
import { PostFormProps } from "@/components/post-form/post-form.type";
...
export const PostForm: FC<PostFormProps> = ({ post }) => {
  ...
  const [postContent, setPostContent] = useState<any[] | null>(null);
  ...
  const handleCreatePost = useCallback(
    async (publish = false) => {
      ...
      try {
        await postService.createOrUpdate(post?.id, {
          title: postTitle,
          content: postContent,
  ...
  return (
    <section className="">
      ...
      <PlateEditor
        value={postContent}
        handleChange={setPostContent}
        initialHtml={JSON.stringify(post?.content)}
      />
